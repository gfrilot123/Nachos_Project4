...
//Functionaility implemented by Chau Cao
	...
	case NumExceptionTypes :
		...
		case PageFaultException:
			...
			//memMap->Mark(0);
			//if global control == Random Pages
			//frameNumber = Random() % 31;
			//else
...
// Finds an available frame to use by searching for the first available
int FrameFinder()
{
	bool frameFree = FALSE;
	int frameNum = -1;
	for(int x = 0; x < 32; x++)
	{
			frameFree = memMap->Test(x);
			if(frameFree != TRUE)
			{
				frameNum = x;
				break;
...
void setValid(int frame)
{
	...
	//{
	//machine->pageTable[frame].virtualPage = frame;
	machine->pageTable[frame].physicalPage =  frame;
	machine->pageTable[frame].valid = true;
	machine->pageTable[frame].use = true;
	//if global control == FIFO,
	pageList->Append((void *) frame);
	pageList->PrintList();
	//int removeResult = (int) pageList->Remove();
	//printf("Remove results in: %d \n", removeResult);
